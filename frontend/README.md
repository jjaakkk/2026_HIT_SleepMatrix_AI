...
- **数据模式**：侧栏"数据模式"默认 `推理接入`（逐帧调用 `/api/frame/analyze`，350ms 节流 + latest-wins），
  推理结果驱动睡姿卡（`label_zh + 置信度`）、热力图（增强矩阵 + 分区掩码/区域）、3D 人体姿势；
  可切换 `数据展示`（记录标签与标注渲染）。后端对不可用模块返回模块级错误对象，仅降级对应模块
  （如睡姿模型缺失 → 睡姿回退记录标签，分区/增强继续用推理结果）；后端离线时每 15s 静默重连，
  重连后自动恢复推理。
- **弱力可视化**：渲染层压扩（γ=0.35）已与后端 `weak_area_enhance` 算法区分命名，口径不混淆。
- dev/preview 均已配置 `/api` 代理 → `127.0.0.1:5000`；生产可用 `VITE_API_BASE` 覆盖。
  演示直链：`display=demo` 进入数据展示、`display=inference` 进入推理接入（兼容旧参数 `pose=svm` / `pose=label`）。
...
| 9 | 推理接入演示（默认模式，需后端在线）：分区掩码/区域+增强热力图+3D 姿势由模型驱动 | `#type=static&person=SAI&action=1&frame=10` |
| 10 | 数据展示模式（记录标签与标注） | `#type=static&person=SAI&action=1&frame=10&display=demo` |
